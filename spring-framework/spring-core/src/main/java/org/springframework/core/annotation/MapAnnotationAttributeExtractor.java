...
/**
 * Implementation of the {@link AnnotationAttributeExtractor} strategy that is
...
class MapAnnotationAttributeExtractor extends AbstractAliasAwareAnnotationAttributeExtractor<Map<String, Object>> {
	 ...
	 * 
	 * @param attributes
	 *            the map of annotation attributes; never {@code null}
	 * @param annotationType
	 *            the type of annotation to synthesize; never {@code null}
	 * @param annotatedElement
	 *            the element that is annotated with the annotation of the
	 *            supplied type; may be {@code null} if unknown
	 */
	...
	/**
	 * Enrich and validate the supplied <em>attributes</em> map by ensuring that
	 * it contains a non-null entry for each annotation attribute in the
	 * specified {@code annotationType} and that the type of the entry matches
	 * the return type for the corresponding annotation attribute.
	 * <p>
	 * If an entry is a map (presumably of annotation attributes), an attempt
	 * will be made to synthesize an annotation from it. Similarly, if an entry
	 * is an array of maps, an attempt will be made to synthesize an array of
	 * annotations from those maps.
	 * <p>
	 * If an attribute is missing in the supplied map, it will be set either to
	 * the value of its alias (if an alias exists) or to the value of the
	 * attribute's default value (if defined), and otherwise an
	 * {@link IllegalArgumentException} will be thrown.
	 */
	@SuppressWarnings("unchecked")
	private static Map<String, Object> enrichAndValidateAttributes(Map<String, Object> originalAttributes,
