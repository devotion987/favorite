...
public class ResolvableType implements Serializable {
	...
	private static final ConcurrentReferenceHashMap<ResolvableType, ResolvableType> cache = new ConcurrentReferenceHashMap<ResolvableType, ResolvableType>(
			256);
	 ...
	 * of the {@link #NONE} constant, this method will never return {@code null}
	 * .
	 */
	public Type getType() {
	...
	/**
	 * Return a {@link ResolvableType} for the specified nesting level. The
	 * nesting level refers to the specific generic parameter that should be
	...
	public boolean equals(Object other) {
		...
		if (this.variableResolver != otherType.variableResolver
				&& (this.variableResolver == null || otherType.variableResolver == null || !ObjectUtils
	 ...
	 * 
	 * @param sourceClass
	 *            the source class ({@code null} is semantically equivalent to
	...
	/**
	 * Return a {@link ResolvableType} for the specified {@link Class} with a
	 * given implementation. For example:
	 ...
	 * @return a {@link ResolvableType} for the specified class backed by the
	 ...
	 * resulting {@link ResolvableType} may not be {@link Serializable}.
	 * 
	 * @param type
	 ...
	 * the given owner type. Note: The resulting {@link ResolvableType} may not
	...
	private static class TypeVariablesVariableResolver implements VariableResolver {
		...
		public ResolvableType resolveVariable(TypeVariable<?> variable) {
			for (int i = 0; i < this.variables.length; i++) {
				if (SerializableTypeWrapper.unwrap(this.variables[i])
						.equals(SerializableTypeWrapper.unwrap(variable))) {
