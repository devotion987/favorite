 ...
 * {@link org.springframework.web.util.UriComponentsBuilder} by pointing to
 * Spring MVC controllers and {@code @RequestMapping} methods.
 *
 * <p>
 * The static {@code fromXxx(...)} methods prepare links relative to the current
 * request as determined by a call to
 * {@link org.springframework.web.servlet.support.ServletUriComponentsBuilder#fromCurrentServletMapping()}
 * .
 *
 * <p>
 * The static {@code fromXxx(UriComponentsBuilder,...)} methods can be given the
 * baseUrl when operating outside the context of a request.
 *
 * <p>
 * You can also create an MvcUriComponentsBuilder instance with a baseUrl via
 * {@link #relativeTo(org.springframework.web.util.UriComponentsBuilder)} and
 * then use the non-static {@code withXxx(...)} method variants.
 *
...
public class MvcUriComponentsBuilder {
	...
	/**
	 * Well-known name for the {@link CompositeUriComponentsContributor} object
	 * in the bean factory.
	 */
	 ...
	 * of the instance based {@code withXxx(...}} methods will create URLs
	 ...
	 * class and current request information including Servlet mapping. If the
	 * controller contains multiple mappings, only the first one is used.
	 * 
	 ...
	 * 
	 * @param builder
	 *            the builder for the base URL; the builder will be cloned and
	...
	/**
	 * Create a {@link UriComponentsBuilder} from the mapping of a controller
	 * method and an array of method argument values. This method delegates to
	 ...
	 *             if there is no matching or if there is more than one matching
	 *             method
	 */
	 ...
	 * useful when using MvcUriComponentsBuilder outside the context of
	...
	/**
	 * Create a {@link UriComponentsBuilder} by invoking a "mock" controller
	 * method. The controller method and the supplied argument values are then
	 * used to delegate to {@link #fromMethod(Class, Method, Object...)}.
	 * <p>
	...
	/**
	 * Create a URL from the name of a Spring MVC controller method's request
	 * mapping.
	 * <p>
	 * The configured
	 ...
	 * separator, and then the method name. For example "PC#getPerson" for a
	 * class named PersonController with method getPerson. In case the naming
	 * convention does not produce unique results, an explicit name may be
	 * assigned through the name attribute of the {@code @RequestMapping}
	 * annotation.
	 * <p>
	 * This is aimed primarily for use in view rendering technologies and EL
	 * expressions. The Spring URL tag library registers this method as a
	 * function called "mvcUrl".
	 * <p>
	 * For example, given this controller:
	 * 
	 * <pre class="code">
	 ...
	 * required to prepare the URL, mainly {@code @RequestParam} and
	 * {@code @PathVariable}).
	 * 
	 * @param mappingName
	 *            the mapping name
	 ...
	 * @throws IllegalArgumentException
	 *             if the mapping name is not found or if there is no unique
	 *             match
	 * @since 4.1
	 */
	...
	public static MethodArgumentBuilder fromMappingName(UriComponentsBuilder builder, String name) {
		...
		if (handlerMethods.size() != 1) {
			throw new IllegalArgumentException(
					"No unique match for mapping mappingName " + name + ": " + handlerMethods);
		}
	...
	/**
	 * Create a {@link UriComponentsBuilder} from the mapping of a controller
	 * method and an array of method argument values. The array of values must
	 * match the signature of the controller method. Values for
	 * {@code @RequestParam} and {@code @PathVariable} are used for building the
	 * URI (via implementations of
	 * {@link org.springframework.web.method.support.UriComponentsContributor
	 * UriComponentsContributor}) while remaining argument values are ignored
	...
	/**
	 * An alternative to {@link #fromMethod(Class, Method, Object...)} that
	 * accepts a {@code UriComponentsBuilder} representing the base URL. This is
	 * useful when using MvcUriComponentsBuilder outside the context of
	 * processing a request or to apply a custom baseUrl not matching the
	 * current request.
	 ...
	 * @since 4.2
	 */
	public static UriComponentsBuilder fromMethod(UriComponentsBuilder baseUrl, Class<?> controllerType, Method method,
			Object... args) {
	...
	private static UriComponentsBuilder fromMethodInternal(UriComponentsBuilder baseUrl, Class<?> controllerType,
	...
	/**
	 * Return a "mock" controller instance. When an {@code @RequestMapping}
	 * method on the controller is invoked, the supplied argument values are
	 * remembered and the result can then be used to create a
	 * {@code UriComponentsBuilder} via {@link #fromMethodCall(Object)}.
	 * <p>
	 * Note that this is a shorthand version of {@link #controller(Class)}
	 * intended for inline use (with a static import), for example:
	 * 
	 * <pre class="code">
	 ...
	 * Return a "mock" controller instance. When an {@code @RequestMapping}
	 * method on the controller is invoked, the supplied argument values are
	 * remembered and the result can then be used to create
	 * {@code UriComponentsBuilder} via {@link #fromMethodCall(Object)}.
	 * <p>
	 * This is a longer version of {@link #on(Class)}. It is needed with
	 * controller methods returning void as well for repeated invocations.
	 * 
	 * <pre class="code">
	...
	/**
	 * An alternative to {@link #fromMethodCall(Object)} for use with an
	...
	/**
	 * An alternative to {@link #fromMappingName(String)} for use with an
	 * instance of this class created via {@link #relativeTo}.
	 * 
	 * @since 4.2
	 */
	...
	/**
	 * An alternative to {@link #fromMethod(Class, Method, Object...)} for use
	 * with an instance of this class created via {@link #relativeTo}.
	 * 
	 * @since 4.2
	 */
	...
	private static class ControllerMethodInvocationInterceptor
			implements net.sf.cglib.proxy.MethodInterceptor, MethodInterceptor {

		private static final Method getControllerMethod = ReflectionUtils.findMethod(MethodInvocationInfo.class,
				"getControllerMethod");

		private static final Method getArgumentValues = ReflectionUtils.findMethod(MethodInvocationInfo.class,
				"getArgumentValues");

		private static final Method getControllerType = ReflectionUtils.findMethod(MethodInvocationInfo.class,
				"getControllerType");
	...
	public static class MethodArgumentBuilder {
		...
		/**
		 * @deprecated as of 4.2, this is deprecated in favor of alternative
		 *             constructors that accept a controllerType argument
		 */
		@Deprecated
		...
		public String build() {
			return fromMethodInternal(this.baseUrl, this.controllerType, this.method, this.argumentValues).build(false)
					.encode().toUriString();
		}

		public String buildAndExpand(Object... uriVars) {
			return fromMethodInternal(this.baseUrl, this.controllerType, this.method, this.argumentValues).build(false)
					.expand(uriVars).encode().toString();
		}
	}
