 ...
 * with its order property set to 0 so that other external resolvers may be
 * ordered before or after it.
 *
 * <p>
 * When content negotiation is enabled the order property is set to highest
 * priority instead with the ContentNegotiatingViewResolver encapsulating all
 * other registered view resolver instances. That way the resolvers registered
 * through the MVC namespace form self-encapsulated resolver chain.
 *
...
public class ViewResolversBeanDefinitionParser implements BeanDefinitionParser {
	...
	public BeanDefinition parse(Element element, ParserContext context) {
		...
		String[] names = new String[] { "jsp", "tiles", "bean-name", "freemarker", "velocity", "groovy",
				"script-template", "bean", "ref" };
		...
		for (Element resolverElement : DomUtils.getChildElementsByTagName(element, names)) {
			...
			if ("jsp".equals(name)) {
				...
				// resolverBeanDef = new
				// RootBeanDefinition(TilesViewResolver.class);
				// addUrlBasedViewResolverProperties(resolverElement,
				// resolverBeanDef);
