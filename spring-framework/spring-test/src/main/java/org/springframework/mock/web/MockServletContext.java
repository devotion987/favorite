 ...
 *
 * <p>
 * Compatible with Servlet 3.0 but can be configured to expose a specific
 * version through {@link #setMajorVersion}/{@link #setMinorVersion}; default is
 * 3.0. Note that Servlet 3.0 support is limited: servlet, filter and listener
 * registration methods are not supported; neither is JSP configuration. We
 * generally do not recommend to unit test your ServletContainerInitializers and
 * WebApplicationInitializers which is where those registration methods would be
 * used.
 *
 * <p>
 ...
 * to configure your {@code MockServletContext} with a
 * {@code FileSystemResourceLoader} in that case to ensure that resource paths
 * are interpreted as relative filesystem locations.
 *
 * <p>
 * A common setup is to point your JVM working directory to the root of your web
 * application directory, in combination with filesystem-based resource loading.
 * This allows to load the context files as used in the web application, with
 * relative paths getting interpreted correctly. Such a setup will work with
 * both {@code FileSystemXmlApplicationContext} (which will load straight from
 * the filesystem) and {@code XmlWebApplicationContext} with an underlying
 * {@code MockServletContext} (as long as the {@code MockServletContext} has
 * been configured with a {@code FileSystemResourceLoader}).
 *
 * @author Rod Johnson
