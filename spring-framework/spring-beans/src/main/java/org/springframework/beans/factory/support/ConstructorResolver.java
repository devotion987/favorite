...
class ConstructorResolver {
    ...
    public BeanWrapper autowireConstructor(final String beanName, final RootBeanDefinition mbd,
        ...
        /** explicitArgs通过getBean{link BeanFactory.getBean(String name, Object... args)}方法传入，如果getBean方法调用的时候指定方法参数那么直接使用 **/
        if (explicitArgs != null) {
            argsToUse = explicitArgs;
        } else {
            /** 如果getBean方法未指定则尝试从配置文件中解析 **/
            ...
            if (argsToResolve != null) {
                /** 解析参数类型，如给定方法的构造函数A(int,int)则通过此方法后会把配置中的("1","1")转换为(1,1)，缓存中的值可能是原始值也可能是最终值 **/
        ...
        if (constructorToUse == null) {
            ...
            for (Constructor<?> candidate : candidates) {
                ...
                if (paramTypes.length < minNrOfArgs) { /** 参数个数不相等 **/
                    continue;
                }
                ...
                /** 探测是否有不确定性的构造函数存在，例如不同构造函数的参数有父子关系 **/
            ...
            if (constructorToUse == null) {
                if (causes != null) {
                    ...
                    causes.forEach(cause -> this.beanFactory.onSuppressedException(cause));
...
//                    }
        ...
        try {
            ...
            if (System.getSecurityManager() != null) {
                ...
                PrivilegedAction<Object> privilegedAction = () -> beanFactory.getInstantiationStrategy().instantiate(
                        mbd, beanName, beanFactory, ctorToUse, argumentsToUse);
                beanInstance = AccessController.doPrivileged(privilegedAction, beanFactory.getAccessControlContext());
            } else {
    ...
    private Method[] getCandidateMethods(final Class<?> factoryClass, final RootBeanDefinition mbd) {
        if (System.getSecurityManager() != null) {
            ...
            PrivilegedAction<Method[]> privilegedAction = () -> (mbd.isNonPublicAccessAllowed() ?
                    ReflectionUtils.getAllDeclaredMethods(factoryClass) : factoryClass.getMethods());
