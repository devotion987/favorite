 ...
 * <p>
 * This class was a {code BeanFactoryPostProcessor} until 1.0.1 version. It
 * changed to {@code BeanDefinitionRegistryPostProcessor} in 1.0.2. See
 ...
 * <p>
 * This class supports filtering the mappers created by either specifying a
 * marker interface or an annotation. The {@code annotationClass} property
 * specifies an annotation to search for. The {@code markerInterface} property
 * specifies a parent interface to search for. If both properties are specified,
 * mappers are added for interfaces that match <em>either</em> criteria. By
 * default, these two properties are null, so all interfaces in the given
 * {@code basePackage} are added as mappers.
 * <p>
 * This configurer enables autowire for all the beans that it creates so that
 * they are automatically autowired with the proper {@code SqlSessionFactory} or
 * {@code SqlSessionTemplate}. If there is more than one
 * {@code SqlSessionFactory} in the application, however, autowiring cannot be
 * used. In this case you must explicitly specify either an
 * {@code SqlSessionFactory} or an {@code SqlSessionTemplate} to use via the
 * <em>bean name</em> properties. Bean names are used rather than actual objects
 * because Spring does not initialize property placeholders until after this
 * class is processed.
 * <p>
 * Passing in an actual object which may require placeholders (i.e. DB user
 * password) will fail. Using bean names defers actual object creation until
 * later in the startup process, after all placeholder substituation is
 * completed. However, note that this configurer does support property
 * placeholders of its <em>own</em> properties. The <code>basePackage</code> and
 * bean name properties all support <code>${property}</code> style substitution.
 * <p>
