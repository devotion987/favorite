 ...
 * provided.<br/>
 *
 * Provided to facilitate the migration from Spring-Batch iBATIS 2 writers to
 * MyBatis 3<br/>
 *
 * The user must provide a MyBatis statement id that points to the SQL statement
 * defined in the MyBatis.<br/>
 *
 * It is expected that {@link #write(List)} is called inside a transaction. If
 * it is not each statement call will be autocommitted and flushStatements will
...
public class MyBatisBatchItemWriter<T> implements ItemWriter<T>, InitializingBean {
	 ...
	 *            a template object for use the {@link SqlSession} on the Spring
	 *            managed transaction
	 */
