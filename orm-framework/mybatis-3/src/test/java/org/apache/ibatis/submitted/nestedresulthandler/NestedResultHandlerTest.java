...
public class NestedResultHandlerTest {
	...
	public void testGetPersonWithHandler() {
		SqlSession sqlSession = sqlSessionFactory.openSession();
		try {
			sqlSession.select("getPersons", new ResultHandler<Object>() {
	...
	public void testUnorderedGetPersonWithHandler() {
		SqlSession sqlSession = sqlSessionFactory.openSession();
		try {
			sqlSession.select("getPersonsWithItemsOrdered", new ResultHandler<Object>() {
				public void handleResult(ResultContext<?> context) {
					Person person = (Person) context.getResultObject();
					if ("grandma".equals(person.getName())) {
						Assert.assertEquals(2, person.getItems().size());
	...
	/**
	 * Fix bug caused by issue #542, see new issue #22 on github If we order by
	 * a nested result map attribute we can miss some records and end up with
	 * duplicates instead.
	 */
	@Test
