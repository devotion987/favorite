...
public class NestedResultHandlerAssociationTest {
	...
	public static void setUp() throws Exception {
		...
		reader = Resources
				.getResourceAsReader("org/apache/ibatis/submitted/nestedresulthandler_association/CreateDB.sql");
	...
	public void shouldHandleRowBounds() throws Exception {
		...
		try {
			sqlSession.select("collectPageByBirthMonth", targetMonth, new RowBounds(1, 2), new ResultHandler<Object>() {
				@Override
	...
	public void shouldHandleStop() throws Exception {
		...
		try {
			Date targetMonth = fmt.parse("2014-01-01");
			sqlSession.select("collectPageByBirthMonth", targetMonth, new ResultHandler<Object>() {
				@Override
				public void handleResult(ResultContext<?> context) {
					Account account = (Account) context.getResultObject();
					accounts.add(account);
